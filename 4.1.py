...
for name, electricity, water in zip(ABONENTS, ELECTRICITY, WATER):
    for i in range(0, 24):
        if electricity[i] > 20 and water[i] > 20:
            print(f"{name} washes  at {i}:00, at this time electricity costs {electricity[i]}, water {water[i]}")
    else:
        print(f"{name} washes at {i}:00, at this time electricity costs {electricity[i]}, water {water[i]}")